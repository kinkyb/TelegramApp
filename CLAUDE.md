...
**GG promo media pool** (since 2026-05-13, cache added 2026-05-15): `_scan_gg_pool(slug)` at [bot.py:102](bot.py:102) delegates to the shared helper [/Users/mac/Desktop/lib/pool_cache.py](../lib/pool_cache.py) `scan_pool(root, exts)`. Listing is cached in `/tmp/of_pool_cache/pool_{key}.json` (6 h TTL, atomic writes, falls back to stale on volume timeout). OFGG uses the same cache file — identical `(root, ext-set)` key — so the rglob actually runs at most once every TTL across both workflows. Per-slug rotation index stored as config key `gg_media_index_{slug}` in `bot.db`, advanced one position per fire (wraps mod pool size). On empty pool the creator_index is rolled back so the same girl is retried on the next slot. R2 key is hash-based (`promo-gifs/{slug}/{md5(relpath)[:12]}{ext}`) so subfolder paths with `@`/spaces never break public URLs, and repeat picks of the same file hit the R2 cache. To force immediate rescan after dropping new media: `rm /tmp/of_pool_cache/pool_*.json` (or wait up to 6 h).
